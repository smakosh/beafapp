...
export const editProfile = (
	data, avatarImage, avatarState, setErrors, resetForm, setSubmitting
)	=> async dispatch => {
	try {
		await dispatch({ type: 'LOADING_PROFILE' })
		...
		if (avatarImage !== avatarState) {
			...
			const res = await axios.post('https://api.cloudinary.com/v1_1/dj8equdxc/image/upload', avatarImage, config)
			const avatar = res.data.secure_url

			await setAuthToken(localStorage.jwtToken)
			await axios.patch(`${REACT_APP_PROD_API}/api/user/edit`, { ...data, avatar })
